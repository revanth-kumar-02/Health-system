...
function DoctorCard({ doc, onBook }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-md hover:border-teal-100/40 transition-all duration-300">
        ...
        <div className="flex items-start gap-4">
          ...
          {!imgError && doc.avatarUrl ? (
            <img 
              ...
              className="w-16 h-16 rounded-full object-cover border-2 border-teal-50 shadow-md flex-shrink-0"
            />
          ) : (
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h4 className="font-bold text-slate-800 text-sm sm:text-base leading-tight">{doc.doctorName}</h4>
            ...
            <p className="text-xs text-medTeal font-bold uppercase tracking-wider mt-0.5">
              {doc.specialization}
            </p>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
      ...
      {selectedDept && (() => {
        ...
        return (
          ...
          >
            {/* Modal Box: Fixed height and flex vertical layout */}
            <div 
              onClick={(e) => e.stopPropagation()}
              className="bg-white rounded-3xl max-w-4xl w-full h-[80vh] max-h-[750px] shadow-2xl border border-teal-50/20 relative flex flex-col overflow-hidden animate-scaleUp"
            >
              
              {/* Header & Stats Block: Fixed at the top */}
              <div className="p-6 md:p-8 pb-4 border-b border-slate-100 flex-shrink-0 relative">
                {/* Close Button */}
                <button
                  onClick={() => setSelectedDept(null)}
                ...
                <div className="mb-4 pr-8">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <h2 className="text-xl md:text-2xl font-extrabold text-slate-800 font-display">{selectedDept.departmentName}</h2>
                    <span className="px-2.5 py-0.5 text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-full">
                      {selectedDept.status}
                    </span>
                  </div>
                  <p className="text-slate-500 text-xs sm:text-sm leading-relaxed max-w-2xl">{selectedDept.description}</p>
                </div>

                {/* Department Statistics Section */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-3 shadow-sm text-center">
                    <Users className="w-5 h-5 text-blue-500 mx-auto mb-0.5" />
                    <p className="text-base font-bold text-slate-800 leading-none">{stats.totalDocs}</p>
                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-1">Total Doctors</p>
                  </div>
                  <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-3 shadow-sm text-center">
                    <CheckCircle className="w-5 h-5 text-emerald-500 mx-auto mb-0.5" />
                    <p className="text-base font-bold text-slate-800 leading-none">{stats.availableDocs}</p>
                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-1">Available Today</p>
                  </div>
                  <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-3 shadow-sm text-center">
                    <Clock className="w-5 h-5 text-purple-500 mx-auto mb-0.5" />
                    <p className="text-xs font-bold text-slate-700 truncate leading-none mt-0.5" title={stats.hoursText}>{stats.hoursText}</p>
                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-1.5">Consulting Hours</p>
                  </div>
                  <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-3 shadow-sm text-center">
                    <Activity className="w-5 h-5 text-amber-500 mx-auto mb-0.5" />
                    <p className="text-base font-bold text-slate-800 leading-none">{stats.avgDuration}m</p>
                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-1">Avg Session Size</p>
                  </div>
                </div>
              </div>

              {/* Scrollable Doctors List Block */}
              <div className="flex-1 overflow-y-auto p-6 md:p-8 pt-4 space-y-4">
                <h3 className="text-sm font-bold text-slate-800 mb-2 font-display flex items-center gap-1.5 border-b border-slate-50 pb-2 flex-shrink-0">
